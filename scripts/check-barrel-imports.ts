...
export type CapabilityBarrelDomain = {
  ...
  name: string;
  /** Repository-relative path to the domain root, e.g. 'apps/daemon/src/http'. */
  root: string;
...
// Reference implementation for the pattern: apps/daemon/src/http/ (see its README.md). The
// original reference module, design-systems/, is mid-migration to this shape on a separate
// branch and is not yet registered here — see http/README.md's "Known limitations" for context.
...
export async function checkCapabilityBarrelImports(): Promise<boolean> {
  ...
  // subdir internals that are deliberately absent from the public barrel. Widening
  // the scan to tests would force those internal-only helpers onto the public
  // surface, defeating the encapsulation this check exists to protect. The
  ...
  // not enforced (see http/README.md → Import conventions).
