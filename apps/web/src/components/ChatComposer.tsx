...
import { subscribeEscapeKey, getViewportSize } from '../providers/composer-dom';
import {
  // Pure rules + formatters.
  ...
  type SlashCommand,
  type ToolsTab,
  type TrackedWorkspaceLinkedDir,
  type TranslateFn,
...
// The composer's public imperative-handle contract now lives in the
// chat-composer slice; re-exported here so external importers (ChatPane,
// ProjectView, SideChatTab) keep importing from './ChatComposer' unchanged.
export type { ChatComposerDraftOptions, ChatComposerHandle, ChatSendMeta };
...
export const ChatComposer = forwardRef<ChatComposerHandle, Props>(
  function ChatComposer(
    ...
    // Draft text + its per-conversation persistence live in the chat-composer
    // slice's hook. It seeds from `initialDraft` (or the stored draft for
    // `draftStorageKey`), persists on change, and returns `draftRef` — the
    // synchronous mirror of `draft` that event handlers mutating off a captured
    // render closure (notably the annotation listener, where two uploads can
    // resolve concurrently) read/write so their edits compose instead of
    // clobbering one another. It is kept in lockstep with `draft` by
    // handleEditorChange (the editor is the single source for typing) and by the
    // programmatic-set paths below.
    const { draft, setDraft, draftRef } = useWiredComposerDraft({ initialDraft, draftStorageKey });
    // Portal host for the slice's dumb components (attachment preview,
    // design-toolbox hover panel). Resolved here (the orchestrator is DOM-capable)
    // and passed down so the slice files stay DOM-free; null under SSR.
          ...
          */}
          ...
          {designSystemPicker || selectedWorkspaceContexts.length > 0 || stagedSkills.length > 0 || stagedMcpServers.length > 0 || stagedConnectors.length > 0 || staged.length > 0 || activeAppliedPlugin ? (
            <StagedRunContexts
              ...
              modalHost={portalHost}
              subscribeEscape={subscribeEscapeKey}
