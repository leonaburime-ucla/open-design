...
Writes <out|cwd>/<id>/{SKILL.md,open-design.json,README.md}.`);
  ...
  const targetDir = typeof flags.out === 'string' && flags.out.length > 0
    ? flags.out
    : process.cwd();
  const { scaffoldPlugin, ScaffoldError } = await import('../../plugins/index.js');
  try {
    const input = {
      targetDir,
      id,
...
Exit codes:
  ...
  let result;
  try {
    const { validatePluginFolder, flattenValidationDiagnostics } = await import('../../plugins/index.js');
    ...
    if (flags.json) {
      ...
      const diagnostics = (await import('../../plugins/index.js')).flattenValidationDiagnostics(result);
...
Exit codes:
  ...
  const folder = rest[0];
  try {
    const { packPlugin, PackPluginError } = await import('../../plugins/index.js');
    let result;
    try {
