...
export async function runPluginSnapshots(args) {
  ...
  if (sub === 'diff') {
    ...
    const { diffSnapshots } = await import('../../plugins/index.js');
...
Exit codes:
  ...
  if (enabledSet.has('simulate')) {
    ...
    if (pipeline && Array.isArray(pipeline.stages) && pipeline.stages.length > 0) {
      const { simulatePipeline } = await import('../../plugins/index.js');
  ...
  // 6. Aggregate.
  const { verifyPlugin } = await import('../../plugins/index.js');
...
Closed signal vocabulary:
  ...
  const { simulatePipeline, parseSignalKv } = await import('../../plugins/index.js');
...
into 'added' / 'removed' / 'changed' with one line per field.`);
  ...
  const a = await respA.json();
  const b = await respB.json();
  const { diffPlugins } = await import('../../plugins/index.js');
