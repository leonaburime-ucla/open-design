...
import { PLUGIN_SHARE_ACTION_LABELS, USER_PLUGIN_SOURCE_KINDS, copyPluginFolderForProjectContext, githubRepoNameFromPluginName, normalizePluginShareAction, renderPluginSharePrompt } from '../../plugins/index.js';
  ...
  return {
    ...
    handlePluginTrust: async (req, res) => {
      try {
        ...
        const { validateCapabilityList, grantCapabilities, revokeCapabilities } = await import('../../plugins/index.js');
        ...
        try { const { recordPluginEvent } = await import('../../plugins/index.js'); recordPluginEvent({ kind: 'plugin.trust-changed', pluginId: req.params.id, details: { action, capabilities: accepted, total: next.length } }); } catch {}
