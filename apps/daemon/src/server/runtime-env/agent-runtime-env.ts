...
/** @module server/runtime-env/agent-runtime-env
 * Environment + prompt construction for agent sub-processes.
 *
 * `buildAgentRuntimeEnv` composes the `NodeJS.ProcessEnv` an agent child is
 * spawned with — sandbox overlay, sidecar IPC passthrough, no-proxy loopback
 * defaults, PATH node-bin injection, and the per-run tool token. It reads two
 * daemon-init singletons (the resolved data dir and the sandbox runtime
 * config); rather than reach back into server.ts (which would cycle), those are
 * passed in explicitly. server.ts owns the singletons and exposes the original
 * `createAgentRuntimeEnv(baseEnv, daemonUrl, toolTokenGrant?, nodeBin?)` public
 * signature as a thin binding wrapper around this.
 *
 * `createAgentRuntimeToolPrompt` is pure (args only) and moves verbatim.
 *
 * Extracted from apps/daemon/src/server.ts (strangler-fig slice 5a). The env
 * body is byte-identical apart from the two singleton references becoming the
 * `runtimeDataDir` / `sandboxRuntime` parameters.
 */

import path from 'node:path';
import { SIDECAR_ENV } from '@open-design/sidecar-proto';
import { applySandboxRuntimeEnv } from '../../sandbox-mode.js';
import { mergeNoProxyWithLoopbackDefaults } from '../../agents/index.js';
